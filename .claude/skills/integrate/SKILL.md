---
name: integrate
description: Absorb the latest lecture's raw notes into the structured chapter/section hierarchy of these discrete mathematics notes. Finds what is new from the git history rather than from any naming convention, so it works wherever the author happened to type. Use when the user has just taken notes and wants them redistributed by topic, matching the existing expository style, boxed environments, numbering, and labels. Triggers on "/integrate", "integrate my notes", "integrate today's lecture", "fold in the last lecture".
---
