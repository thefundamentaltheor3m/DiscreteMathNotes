...
- **The corpus's own idioms.** These notes and the four sibling repositories that
  ...
  [LieAlgebrasNotes][a] — have settled conventions. Writing $v \in H$ for a vertex of
  a hypergraph, or $f \in H$ for an edge, is an abuse of notation the author uses
  deliberately and consistently. Grep the corpus before calling one an error: an
...
discrete mathematics course. Read whatever you need from it — the files under
...
```bash
gh api repos/thefundamentaltheor3m/DiscreteMathNotes/pulls/<n>/comments \
...
```bash
gh api repos/thefundamentaltheor3m/DiscreteMathNotes/pulls/<n>/comments/<id>/replies \
