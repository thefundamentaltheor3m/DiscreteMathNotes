%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CUSTOM COMMANDS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% `sorry`

\newcommand{\sorry}{\textcolor{red}{\texttt{sorry}}}

% TIKZ:

\newcommand{\drawplane}{  % For use with TikZ
    \draw[step=0.5cm,gray,very thin] (-3,-3) grid (3,3);
    \draw[thick,->] (-3,0) -- (3,0) node[anchor=west] {$x$};
    \draw[thick,->] (0,-3) -- (0,3) node[anchor=south] {$y$};
    \node[black, anchor=north east] at (0, 0) {$0$};
}
\usetikzlibrary{decorations.markings}
\tikzset{->-/.style={decoration={
      markings,
      mark=at position #1 with {\arrow{>}}},postaction={decorate}
}}

\newenvironment{cd}{
    \begin{equation} \begin{tikzcd}
}{
    \end{tikzcd} \end{equation}
}
\newenvironment{cd*}{
    \begin{equation*} \begin{tikzcd}
}{
    \end{tikzcd} \end{equation*}
}

\newcommand{\drawsquare}[1]{
    \draw[thick, blue]
    (-{#1},-{#1}) node [anchor=north east] {C} --
    (-{#1}, {#1}) node[anchor=south east] {B} --
    ({#1}, {#1}) node[anchor=south west] {A} --
    ({#1}, -{#1}) node [anchor=north west] {D} -- cycle;
}

% DELIMITERS:

\newcommand{\parenth}[1]{\left( #1 \right)}
\newcommand{\brac}[1]{\left[ #1 \right]}
\newcommand{\set}[1]{\left\{ #1 \right\}}
\newcommand{\setst}[2]{\set{#1 \; \middle\vert \; #2}}
\newcommand{\abs}[1]{\left\lvert #1 \right\rvert}
\newcommand{\norm}[1]{\left\lVert #1 \right\rVert}
\newcommand{\floor}[1]{\left\lfloor #1 \right\rfloor}
\newcommand{\ceil}[1]{\left\lceil #1 \right\rceil}
\newcommand{\cycl}[1]{\left\langle #1 \right\rangle}
\newcommand{\grpres}[2]{\cycl{#1 \; \middle| \; #2}}  % \grpres{gens}{rels}

% FUNCTIONS:

\newcommand{\fx}{f\!\parenth{x}}
\newcommand{\fof}[1]{f\!\parenth{#1}}

\newcommand{\px}{p\!\parenth{x}}
\newcommand{\pof}[1]{p\!\parenth{#1}}
\newcommand{\pofbig}[1]{p\Big(#1\Big)}
\newcommand{\gx}{g\!\parenth{x}}
\newcommand{\gof}[1]{g\!\parenth{#1}}
\newcommand{\hx}{h\!\parenth{x}}
\newcommand{\hof}[1]{h\!\parenth{#1}}
\newcommand{\Tv}{T\!\parenth{v}}
\newcommand{\Tof}[1]{T\!\parenth{#1}}
\newcommand{\Tbar}{\overline{T}}
\newcommand{\Tbarof}[1]{\Tbar\!\parenth{#1}}
\newcommand{\Tbarv}{\Tbarof{v}}
\newcommand{\Sof}[1]{S\!\parenth{#1}}

\newcommand{\psin}[1]{\sin\!{\parenth{#1}}}
\newcommand{\pcos}[1]{\cos\!{\parenth{#1}}}
\newcommand{\ptan}[1]{\tan\!{\parenth{#1}}}
\newcommand{\sinsq}[1]{\sin^2\!\parenth{#1}}
\newcommand{\cossq}[1]{\cos^2\!\parenth{#1}}
\newcommand{\tansq}[1]{\tan^2\!\parenth{#1}}
\newcommand{\parcsin}[1]{\arcsin\!{\parenth{#1}}}
\newcommand{\parccos}[1]{\arccos\!{\parenth{#1}}}
\newcommand{\parctan}[1]{\arctan\!{\parenth{#1}}}

\newcommand{\logbase}[2]{\log_{#1}\!\parenth{#2}}
\newcommand{\nthroot}[2]{\sqrt[\leftroot{-3}\uproot{3} #1]{#2}}
\newcommand{\cbrt}[1]{\nthroot{3}{#1}}

\newcommand{\pgcd}[2]{\gcd\!\parenth{{#1},{#2}}}
\newcommand{\plcm}[2]{\operatorname{lcm}\!\parenth{{#1},{#2}}}
\newcommand{\pgcds}[1]{\gcd\!\parenth{#1}}
\newcommand{\plcms}[1]{\operatorname{lcm}\!\parenth{#1}}

\newcommand{\varphiof}[1]{\varphi\!\parenth{#1}}
\newcommand{\phiof}[1]{\phi\!\parenth{#1}}
\newcommand{\xiof}[1]{\xi\!\parenth{#1}}
\newcommand{\rhoof}[1]{\rho\!\parenth{#1}}
\newcommand{\pexp}[1]{\exp\!\parenth{#1}}

\newcommand{\inj}{\hookrightarrow}
\newcommand{\surj}{\twoheadrightarrow}
\newcommand{\iso}{\xrightarrow{\sim}}

\newcommand{\of}[1]{\!\parenth{#1}}

% CALCULUS:

\newcommand{\dx}{\operatorname{d}\!x}
\newcommand{\dy}{\operatorname{d}\!y}
\newcommand{\diff}[1]{\operatorname{d}\!{#1}}
\newcommand{\dydx}{\frac{\dy}{\dx}}

% LINEAR ALGEBRA:

\newcommand{\mat}[3]{\operatorname{M}_{{#1} \times {#2}}\!\parenth{{#3}}}
\newcommand{\matsq}[2]{\operatorname{M}_{{#1} \times {#1}}\!\parenth{\mathbb{#2}}}
\newcommand{\MnR}{\operatorname{M}_{n}\!\parenth{\real}}
\newcommand{\MnC}{\operatorname{M}_{n}\!\parenth{\C}}
\newcommand{\Mn}[2]{\operatorname{M}_{#1}\!\parenth{#2}}
\newcommand{\GL}[1]{\operatorname{GL}\!\parenth{#1}}
\newcommand{\SL}[1]{\operatorname{SL}\!\parenth{#1}}
\newcommand{\PGL}[1]{\operatorname{PGL}\!\parenth{#1}}
\newcommand{\PSL}[1]{\operatorname{PSL}\!\parenth{#1}}

\newcommand{\Span}[1]{\operatorname{Span}\!\parenth{#1}}

\newcommand{\pdim}[1]{\dim\!\parenth{#1}}

\newcommand{\RSp}[1]{\operatorname{RSp}\!\parenth{#1}}
\newcommand{\CSp}[1]{\operatorname{CSp}\!\parenth{#1}}
\newcommand{\rank}[1]{\operatorname{rank}\!\parenth{#1}}
\newcommand{\pim}[1]{\operatorname{im}\!\parenth{#1}}
\newcommand{\pker}[1]{\operatorname{ker}\!\parenth{#1}}
\newcommand{\pdet}[1]{\det\!\parenth{#1}}

\newcommand{\cA}[1]{c_A \! \parenth{#1}}
\newcommand{\cB}[1]{c_B \! \parenth{#1}}
\newcommand{\mA}[1]{m_A \! \parenth{#1}}
\newcommand{\mB}[1]{m_B \! \parenth{#1}}
\newcommand{\cAx}{\cA{x}}
\newcommand{\cBx}{\cB{x}}
\newcommand{\mAx}{\mA{x}}
\newcommand{\mBx}{\mB{x}}
\newcommand{\cof}[2]{c_{#1}\!\parenth{#2}}
\newcommand{\mof}[2]{m_{#1}\!\parenth{#2}}

\newcommand{\Cof}[1]{C\!\parenth{#1}}
\newcommand{\+}{\oplus}
\newcommand{\pdeg}[1]{\deg\!\parenth{#1}}

\newcommand{\RCF}[1]{\operatorname{RCF}\!\parenth{#1}}
\newcommand{\JCF}[1]{\operatorname{JCF}\!\parenth{#1}}

\newcommand{\Jsub}[2]{J_{#1}\!\parenth{#2}}

\newcommand{\Aof}[1]{A\!\parenth{#1}}

\newcommand{\Tsub}[2]{T_{#1}\!\parenth{#2}}

\newcommand{\Tr}[1]{\operatorname{Tr}\!\parenth{#1}}

\newcommand{\diag}[1]{\operatorname{diag}\!\parenth{#1}}

% Measure Theory

\newcommand{\muof}[1]{\mu\!\parenth{#1}}
\newcommand{\muA}{\muof{A}}
\newcommand{\muB}{\muof{B}}
\newcommand{\mutof}[1]{\Tilde{\mu}\!\parenth{#1}}
\newcommand{\mustof}[1]{\mu^*\!\parenth{#1}}
\newcommand{\calF}{\mathcal{F}}
\newcommand{\calA}{\mathcal{A}}
\newcommand{\calB}{\mathcal{B}}
\newcommand{\calC}{\mathcal{C}}
\newcommand{\sigmaof}[1]{\sigma\!\parenth{#1}}
\newcommand{\psiof}[1]{\psi\!\parenth{#1}}

% Intervals

\newcommand{\oc}[2]{\left(#1, #2\right]}
\newcommand{\co}[2]{\left[#1, #2\right)}

% GROUPS/ALGEBRA IN GENERAL:

\newcommand{\inv}{^{-1}}
\newcommand{\Sym}[1]{\operatorname{Sym}\!\parenth{#1}}
\newcommand{\ord}[1]{\operatorname{ord}\!\parenth{#1}}
\newcommand{\supp}[1]{\operatorname{supp}\!\parenth{#1}}
\newcommand{\sgn}[1]{\operatorname{sgn}\!\parenth{#1}}
\newcommand{\tcyc}[2]{\begin{pmatrix} #1 & #2 \end{pmatrix}}

\newcommand{\nsg}{\trianglelefteq}

\newcommand{\Rmul}{R^{\times}}
\newcommand{\kmul}{k^{\times}}
\newcommand{\kX}{k\!\brac{X}}
\newcommand{\RX}{R\!\brac{X}}

\newcommand{\quotient}[2]{
    \left.\raisebox{.2em}{${#1}$} \middle/ \raisebox{-.2em}{${#2}$} \right.
}

\newcommand{\Vof}[1]{V\!\parenth{#1}}

\newcommand{\Frac}[1]{\operatorname{Frac}\!\parenth{#1}}

\newcommand{\Spec}[1]{\operatorname{Spec}\!\parenth{#1}}

\newcommand{\id}{\operatorname{id}}

\newcommand{\pchar}[1]{\operatorname{char}\!\parenth{#1}}

\newcommand{\Hom}{\operatorname{Hom}}

\newcommand{\Zof}[1]{\operatorname{Z}\!\parenth{#1}}

\newcommand{\Aut}[1]{\operatorname{Aut}\!\parenth{#1}}

% NUMBER SETS:

\newcommand{\real}{\mathbb{R}}
\newcommand{\rational}{\mathbb{Q}}
\newcommand{\naturalnum}{\mathbb{N}}
\newcommand{\integers}{\mathbb{Z}}
\newcommand{\complex}{\mathbb{C}}

\newcommand{\F}{\mathbb{F}}
\newcommand{\Q}{\mathbb{Q}}
\newcommand{\R}{\real}
\newcommand{\N}{\naturalnum}
\newcommand{\Z}{\integers}
\newcommand{\C}{\complex}
\newcommand{\B}{\mathcal{B}}
\newcommand{\I}{\mathcal{I}}
\newcommand{\J}{\mathcal{J}}

\newcommand{\psup}[1]{\sup\!\parenth{#1}}
\newcommand{\pinf}[1]{\inf\!\parenth{#1}}

% LOGIC:

\newcommand{\ergo}{\therefore}
\newcommand{\bcos}{\because}
\newcommand{\st}{\text{ s.t. }}

% DISCRETE MATH COURSE

\newcommand{\dist}[1]{\operatorname{dist}\!\parenth{#1}}

% The indicator random variable of an event.
\newcommand{\ind}[1]{\mathbf{1}_{#1}}

% The hyper-edge set of the hypergraph H, and likewise \mathcal{G}, \mathcal{F}, ... for any other
% letter. This takes over LaTeX's own \H, which is the double acute accent of Erd\H{o}s; the accent
% is kept under the name \Hacute, and \Erdos at the foot of this file is how to write the name.
% (Guarding the two meanings with \ifmmode instead does not work: amsmath measures the cells of an
% align environment outside maths mode, so \H inside one would take the text branch and break.)
\let\Hacute\H
\renewcommand{\H}{\mathcal{H}}
% inputenc maps the literal characters ő and Ő to \H o and \H O, so they would expand to
% $\mathcal{H}$o here. Point them at the saved accent, so typing either straight into the
% source (in Overleaf, say) still works.
\DeclareUnicodeCharacter{0151}{\Hacute{o}}
\DeclareUnicodeCharacter{0150}{\Hacute{O}}

\newcommand{\cyclevertices}[2][1]{  % [radius]{n}: the n vertices of a regular n-gon
    \foreach \i in {1, ..., #2}{
        \pgfmathsetmacro{\angthis}{90 + 360 * (\i - 1) / #2}
        \fill[black] (\angthis:#1) circle (2pt);
    }
}

\newcommand{\stepedges}[3][1]{  % [radius]{n}{step}: the edges joining each vertex of a regular n-gon to the one `step' further round
    \foreach \i in {1, ..., #2}{
        \pgfmathsetmacro{\angthis}{90 + 360 * (\i - 1) / #2}
        \pgfmathsetmacro{\angnext}{90 + 360 * (\i - 1 + #3) / #2}
        \draw[thick] (\angthis:#1) -- (\angnext:#1);
    }
}

\newcommand{\cyclegraph}[2][1]{  % [radius]{n}: the cycle graph on n vertices, drawn as a regular n-gon
    \stepedges[#1]{#2}{1}
    \cyclevertices[#1]{#2}
}

\newcommand{\fanoplane}[1][1.6]{  % [radius]: the Fano plane, drawn as a triangle with its medians and the circle through its edge midpoints
    \pgfmathsetmacro{\rmid}{0.5 * #1}
    \foreach \i in {0, 1, 2}{
        \pgfmathsetmacro{\angcorner}{90 + 120 * \i}
        \pgfmathsetmacro{\angnext}{210 + 120 * \i}
        \pgfmathsetmacro{\angopp}{270 + 120 * \i}
        \draw[thick] (\angcorner:#1) -- (\angnext:#1);
        \draw[thick] (\angcorner:#1) -- (\angopp:\rmid);
    }
    \draw[thick] (0, 0) circle (\rmid);
    \foreach \i in {0, 1, 2}{
        \pgfmathsetmacro{\angcorner}{90 + 120 * \i}
        \pgfmathsetmacro{\angmid}{150 + 120 * \i}
        \fill[black] (\angcorner:#1) circle (2pt);
        \fill[black] (\angmid:\rmid) circle (2pt);
    }
    \fill[black] (0, 0) circle (2pt);
}

\newcommand{\stargraph}[2][1]{  % [radius]{n}: the star graph with n edges, drawn as a hub with n spokes
    \foreach \i in {1, ..., #2}{
        \pgfmathsetmacro{\angthis}{90 + 360 * (\i - 1) / #2}
        \draw[thick] (0, 0) -- (\angthis:#1);
        \fill[black] (\angthis:#1) circle (2pt);
    }
    \fill[black] (0, 0) circle (2pt);
}

\newcommand{\cyclearc}[4][1]{  % [radius]{n}{k}{s}: the n-cycle as n dots on a circle, with the k-interval running from vertex s highlighted
    \pgfmathtruncatemacro{\klast}{#3 - 1}
    \pgfmathsetmacro{\angfrom}{90 + 360 * (#4 - 1) / #2}
    \pgfmathsetmacro{\angto}{90 + 360 * (#4 - 2 + #3) / #2}
    \draw[gray!50] (0, 0) circle (#1);
    \draw[very thick, magenta] (\angfrom:#1) arc (\angfrom:\angto:#1);
    \foreach \i in {1, ..., #2}{
        \pgfmathsetmacro{\angthis}{90 + 360 * (\i - 1) / #2}
        \fill[black] (\angthis:#1) circle (2pt);
    }
    \foreach \j in {0, ..., \klast}{
        \pgfmathsetmacro{\angdot}{90 + 360 * (#4 - 1 + \j) / #2}
        \fill[magenta] (\angdot:#1) circle (3pt);
    }
}

\newcommand{\cyclefan}[4][1]{  % [radius]{n}{k}{s}: the n-cycle, with the k intervals through vertex s drawn as concentric arcs outside it
    \pgfmathtruncatemacro{\klast}{#3 - 1}
    \draw[gray!50] (0, 0) circle (#1);
    \foreach \i in {1, ..., #2}{
        \pgfmathsetmacro{\angthis}{90 + 360 * (\i - 1) / #2}
        \fill[black] (\angthis:#1) circle (2pt);
    }
    \foreach \j in {0, ..., \klast}{
        \pgfmathsetmacro{\rarc}{#1 + 0.17 * (\j + 1)}
        \pgfmathsetmacro{\angfrom}{90 + 360 * (#4 - #3 + \j) / #2}
        \pgfmathsetmacro{\angto}{90 + 360 * (#4 - 1 + \j) / #2}
        \draw[thick, magenta] (\angfrom:\rarc) arc (\angfrom:\angto:\rarc);
    }
    \pgfmathsetmacro{\angx}{90 + 360 * (#4 - 1) / #2}
    \fill[magenta] (\angx:#1) circle (3pt);
}

\newcommand{\coloredcyclegraph}[3][1]{  % [radius]{n}{colors}: the cycle graph on n vertices, with its vertices colored in turn by the n colors in the comma-separated list
    \stepedges[#1]{#2}{1}
    \foreach \c [count = \i] in {#3}{
        \pgfmathsetmacro{\angthis}{90 + 360 * (\i - 1) / #2}
        \fill[\c] (\angthis:#1) circle (3pt);
    }
}

\newcommand{\coloredstargraph}[4][1]{  % [radius]{n}{hub color}{leaf color}: the star graph with n edges, its hub and its leaves colored as named
    \foreach \i in {1, ..., #2}{
        \pgfmathsetmacro{\angthis}{90 + 360 * (\i - 1) / #2}
        \draw[thick] (0, 0) -- (\angthis:#1);
        \fill[#4] (\angthis:#1) circle (3pt);
    }
    \fill[#3] (0, 0) circle (3pt);
}

% {e_1}{e_2}{e_3}{label}: a graph on three vertices with the given label underneath, for the
% exposure tree of 3.4.1. Unlike the graph macros above this one is a whole picture rather than
% a fragment, because it is used as the contents of a node in a larger tikzpicture. Each edge is
% `unexposed' (not looked at yet, so there with probability a half: drawn dashed), `present'
% (drawn solid) or `absent' (not drawn at all); $e_1$ joins the two bottom vertices, $e_2$ the
% left one to the apex, $e_3$ the right one.
\newcommand{\exposedtriad}[4]{%
    \shortstack{%
        \begin{tikzpicture}[
            present/.style = {thick, black},
            unexposed/.style = {dashed, black!55, dash pattern = on 1.6pt off 1.4pt},
            absent/.style = {draw = none},
            dot/.style = {circle, fill = black, inner sep = 1.1pt}]
            \node[dot] (tl) at (0, 0) {};
            \node[dot] (tr) at (0.7, 0) {};
            \node[dot] (ta) at (0.35, 0.55) {};
            \draw[#1] (tl) -- (tr);
            \draw[#2] (tl) -- (ta);
            \draw[#3] (tr) -- (ta);
        \end{tikzpicture}%
        \\[1pt] $#4$%
    }%
}

\newcommand{\BigO}[1]{\operatorname{O}\of{#1}}

\newcommand{\Adj}{\operatorname{Adj}}

\newcommand{\ssq}{\subseteq}

\newcommand{\E}{\mathbb{E}}
\newcommand{\Var}[1]{\operatorname{Var}\of{#1}}

\newcommand{\bipartiteparts}[3][0.9]{  % [spacing]{m}{n}: the two parts of a bipartite graph, as columns of m and n vertices named (A1), ..., (Am) and (B1), ..., (Bn), ready to be joined by \draw (A1) -- (B2);
    \foreach \i in {1, ..., #2}{
        \pgfmathsetmacro{\yleft}{#1 * ((#2 + 1) / 2 - \i)}
        \node[circle, fill = black, inner sep = 1.5pt] (A\i) at (0, \yleft) {};
    }
    \foreach \j in {1, ..., #3}{
        \pgfmathsetmacro{\yright}{#1 * ((#3 + 1) / 2 - \j)}
        \node[circle, fill = black, inner sep = 1.5pt] (B\j) at (3, \yright) {};
    }
}

\newcommand{\Erdos}{Erd\Hacute{o}s}
\newcommand{\girth}{\operatorname{girth}}
\newcommand{\eps}{\varepsilon}