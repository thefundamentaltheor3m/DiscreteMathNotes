...
% so $D$ is a region of the plane. Under $\ssq \R$ the embedding lands on a line, which $n$ points drawn from a
% square miss with probability $1$, and the lemma below would be false for every $\delta > 0$ rather than merely
% vacuous.
% [SUSPECT] "any $D \ssq \R^2$" is still too generous for the lemma below: the same measure-zero argument kills
% any $D$ that is a finite set or a curve, so $D$ wants nonempty interior --- a region rather than a subset. Left
% as written pending the author and the cited paper.
...
\begin{boxtheorem}[\Lovasz Local Lemma]
    ...
    Given a collection $\mathcal{A}$ of events and real numbers $0 < x_A < 1$ such that for all $A \in \mathcal{A}$,
    \begin{align*}
