...
Back to coloring vertices, and this time we are not rationed to two colors. \Cref{Ch2:CH} opened by asking which graphs can be colored with two of them; the question now is how many colors a graph needs in general, and how far that number can be pushed away from the obvious lower bound.
...
The proof of \Cref{Ch2:Thm:Erdos_Girth} plays the same trick, but with a random graph (\Cref{Ch1:Def:Random_Graph}) in place of a random set of vertices.
