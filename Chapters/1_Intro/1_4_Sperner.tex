...
A hypergraph is a vertex set together with a family of subsets of it, and the last question this chapter asks about one is how large that family can be. Unrestricted the question is dull---all $2^n$ subsets of $[n]$ form a hypergraph---so we forbid the cheapest way of making a family large, which is to pile sets up inside one another.
...
% [CORRECTED] was: "perfect matchings between the $k$th layer and the $(k + 1)$th layer" --- consecutive layers have
% equal size only when $n = 2k + 1$, so a matching saturating both sides fails already at $k = 0$. What exists for
% every $k < n/2$ is a complete matching of the smaller, lower layer, the two layers spanning a biregular graph with
% $d_1 = n - k$ and $d_2 = k + 1$.
An intuitive way of arguing is to find complete matchings of the $k$th layer into the $(k + 1)$th layer (cf. \Cref{Ch1:Cor:Biregular}), but at some point, this starts to break down. And at what point is that? Before $k$ exceeds $n / 2$.

\begin{proof}
    Let $\H$ be a Sperner system on $[n]$. Define
    \begin{align*}
        \calF = \set{\emptyset, \set{1}, \set{1, 2}, \set{1, 2, 3}, \ldots, \set{1, 2, \ldots, n}}
    \end{align*}
    ...
    \begin{align*}
        X = \abs{\setst{f \in \calF}{f \in \H_{\pi}}}
    \end{align*}
    Note that we can express
    \begin{align*}
        X = \sum_{h \in \H} \xi_{h, \pi}
    \end{align*}
    where $\xi_{h, \pi}$ is $1$ if $\pi(h) \in \calF$ and $0$ otherwise.
    ...
    The conclusion now comes from bounding $\E\of{X}$ on the other side. Any $\pi$ is a bijection and so preserves containment, which makes $\H_{\pi}$ a Sperner system; and $\calF$ is a chain, any two of its members being comparable. A chain and a Sperner system share at most one set, so $X \leq 1$ for every $\pi$, and hence $\E\of{X} \leq 1$. Since $\binom{n}{k}$ is largest at $k = \floor{n / 2}$, this gives
    \begin{align*}
