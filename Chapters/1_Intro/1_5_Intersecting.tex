...
The result is \Erdos, Ko and Rado's~\cite{ErdosKoRado}.
...
Now let's prove the result in full generality, by an argument of Katona's~\cite{Katona}. The random permutation survives, but the partition into $n / k$ blocks does not, and what replaces it is a cycle.
...
The usual route to this is Hilton and Milner's theorem~\cite{HiltonMilner}, which bounds an intersecting family whose members share no common vertex well below $\binom{n-1}{k-1}$; the argument above gets there without it, at the cost of the bookkeeping over transpositions.
