...
The question is: how big can an intersecting $k$-uniform hypergraph on $n$ vertices be? Ie, how many hyper-edges can it have? Small $k$ is worth doing by hand, because every case points the same way.
...
As a warmup case, let's assume that $k \mid n$. Suppose $H$ is intersecting. Let $\calF$ denote some partition of $H$ into subsets of size $k$ (so $\calF$ is fundamentally a collection of subsets).
\begin{align*}
    X = \abs{\setst{h \in \H}{h \in \pi(f) \text{ for some } f \in \calF}}
\end{align*}
...
\begin{proof}[Proof of \Cref{Ch1:Thm:EKR}]
...
That argument has slack in exactly one place, and at $\abs{\H} = \binom{n-1}{k-1}$ there is none left: $X$ is then equal to $k$ for every ordering rather than merely on average, so every ordering of the cycle hands us a run of $k$ intervals all of which are hyper-edges. That is enough to pin $\H$ down completely.
...
Whatever order we lay the vertices out in, the picture looks like the one above: the hyper-edges we can see on the cycle are the $k$ intervals through one distinguished vertex, drawn here as nested arcs. All the work is in showing that the distinguished vertex does not move when the order changes.
