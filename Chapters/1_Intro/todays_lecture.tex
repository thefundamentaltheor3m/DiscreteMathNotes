...
% Not from the lecture: Markov's inequality was used by name in 1.1.4 and 1.3.4 and never stated; the statement and proof below were supplied.
...
\begin{boxtheorem}[Chebyshev's Inequality] \label{Ch3:Thm:Chebyshev}
    Let $X$ be a random variable and let $a \geq 0$. Then,
    \begin{align*}
...
\end{align*}
Indeed, fix a vertex $x$. $\deg(x)$ is a binomial random variable, counting the $n - 1$ possible edges at $x$, each present independently. $\E\of{\deg(x)} = \frac{n-1}{2}$ and $\Var{\deg(x)} = \frac{n-1}{2}$.
...
\begin{align*}
    ...
    = \frac{e^t + e^{-t}}{2}
\end{align*}
The first inequality is the whole content of the extension: $h$ is the chord of $x \mapsto e^{tx}$ across $[-1, 1]$, and since $e^{tx}$ is convex it lies below that chord on the whole interval, which is where $\abs{\xi_i} \leq 1$ is used. The second equality is linearity, which is where $\E\of{\xi_i} = 0$ is used.
