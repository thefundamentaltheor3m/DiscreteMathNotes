...
We say two events $A$ and $B$ are \textbf{independent} if $\Pr\of{A \cap B} = \Pr(A) \cdot \Pr(B)$. We say that random variables $X_1, \ldots, X_n$ are independent if for all Borel sets $A_1, \ldots, A_n \ssq \R$ the events $X_1\inv\brac{A_1}, \ldots, X_n\inv\brac{A_n}$ (all of which lie in $\mathcal{F}$ because the $X_i$ are measurable) satisfy $\Pr\of{X_1\inv\brac{A_1} \cap \cdots \cap X_n\inv\brac{A_n}} = \Pr\of{X_1\inv\brac{A_1}} \cdots \Pr\of{X_n\inv\brac{A_n}}$. The expectation is the integral of a random variable over the probability space with respect to the probability measure.
