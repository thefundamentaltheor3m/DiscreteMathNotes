...
\begin{boxexample}[Some Adjacency Matrices of Hypergraphs] \hfill
    \begin{enumerate}
        ...
        \begin{align*}
            ...
            \end{bmatrix}
        \end{align*}
        Reading the columns off one at a time gives $\set{1}$, $\set{1}$, $\set{1, 2, 3}$, $\set{1, 2}$ and $\set{3}$, and the first two of those are the same set. So what the matrix describes is the hypergraph on three vertices with the four hyper-edges $f_1 = \set{1}$, $f_2 = \set{1, 2, 3}$, $f_3 = \set{1, 2}$ and $f_4 = \set{3}$, taken in the order the columns give them.
        \begin{center}
            \begin{tikzpicture}
