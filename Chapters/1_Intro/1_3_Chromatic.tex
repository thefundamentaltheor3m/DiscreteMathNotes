...
Back to coloring vertices, and this time we are not rationed to two colors. \Cref{Ch1:CH} opened by asking which graphs can be colored with two of them; the question now is how many colors a graph needs in general, and how far that number can be pushed away from the obvious lower bound.

\begin{boxdefinition}[Chromatic Number]
    The \textbf{chromatic number} of a graph $G$, denoted $\chi(G)$, is the smallest number of colors I need to color $G$.
\end{boxdefinition}
...
% of the $2$-coloring theorem in 1.1.1 colors each component separately), and on that reading every path would be a clique in itself, so $\omega(P_n) = n$
...
\begin{boxproposition} \label{Ch1:Prop:Zykov}
...
The graphs of \Cref{Ch1:Prop:Zykov} have no triangles, but nothing stops them having cycles of length four or five---$G_3$ is one. It is natural to ask whether forbidding all short cycles finally holds the chromatic number down, and the answer is that it does not.
...
\begin{boxtheorem}[\Erdos] \label{Ch1:Thm:Erdos_Girth}
...
The proof of \Cref{Ch1:Thm:Erdos_Girth} plays the same trick, but with a random graph in place of a random set of vertices.

% Not from the lecture: the lecture used $G_{n, p}$ without defining it, and the definition below was supplied.
\begin{boxdefinition}[Random Graph] \label{Ch1:Def:Random_Graph}
...
\begin{proof}[Proof of \Cref{Ch1:Thm:Erdos_Girth}]
    ...
    % [CLAUDE] Blocked on the author, so skip this unless the condition below is met. The directive here was:
    % "you may need to replace some of what you've written with what we did today (4 September 2026)".
    % Nothing from that lecture replaces the continuation supplied below: it went from Markov and Chebyshev to
    % counting triangles in $G_{n, p}$, and never came back to this union bound. Act on it only once the
    % remainder of that lecture (cut short mid-session) has been typed up.
