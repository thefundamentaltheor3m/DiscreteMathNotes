...
The two inequalities this chapter runs on both bound a tail probability in terms of the expectation, and both are proved in a line.
...
\end{boxtheorem}
\begin{proof}
    The event $\abs{X - \E(X)} \geq a$ is the event $\parenth{X - \E(X)}^2 \geq a^2$, and $\parenth{X - \E(X)}^2$ is non-negative with expectation $\Var{X}$. So \Cref{Ch3:Thm:Markov}, applied to it at $a^2$, is the bound.
\end{proof}
...
\end{boxcorollary}
\begin{proof}
    If $X_n = 0$ then $\abs{X_n - \E(X_n)} = \E(X_n)$, so $\Pr\of{X_n = 0} \leq \Pr\of{\abs{X_n - \E(X_n)} \geq \E(X_n)}$, and \Cref{Ch3:Thm:Chebyshev} at $a = \E(X_n)$ bounds that by $\frac{\Var{X_n}}{\E(X_n)^2}$ for every such $n$, which tends to $0$.
\end{proof}
...
\begin{boxexample}
    Flip $n$ fair coins and let $X$ be the number of heads. Then $\E(X) = \frac{n}{2}$, and as $X$ is a sum of $n$ independent variables of variance $\frac{1}{4}$ each, $\Var{X} = \frac{n}{4}$. So \Cref{Ch3:Thm:Chebyshev} at $a = \eps n$ gives
    \begin{align*}
